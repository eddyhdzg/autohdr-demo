// Inject TikTok icon into page footer (Mintlify doesn't support TikTok natively)
(function () {
  var TIKTOK_SVG =
  ...
  function injectFooterTikTok() {
    ...
    var instagramLink = footer.querySelector("a[href*='instagram']");
    if (instagramLink && instagramLink.nextSibling) {
      instagramLink.parentElement.insertBefore(
        tiktokLink,
        instagramLink.nextSibling
      );
    } else {
      socialLinks[socialLinks.length - 1].parentElement.appendChild(tiktokLink);
    }
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", injectFooterTikTok);
  } else {
    injectFooterTikTok();
  }
